...
import { ArrowLeft, Download, FileText, CheckCircle2, ShieldCheck, Database, GitBranch, ArrowRight } from "lucide-react";
import { motion } from "framer-motion";
import { versionRegistry } from "@/data/versions";

const product = versionRegistry.find(v => v.id === "greatgoga")!;

export default function GreatGogaDetail() {
  return (
    <div className="max-w-5xl mx-auto px-6 pt-32 pb-24 relative min-h-screen">
      {/* Background radial glow */}
      <div className="absolute top-10 left-10 w-96 h-96 bg-indigo-600/5 blur-3xl pointer-events-none" />
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            {["Kotlin", "Room DB", "gRPC Protocol", "Firebase Auth", "WorkManager"].map(tag => (
          ...
          <div className="flex flex-wrap gap-4 pt-4">
            ...
            <Link
              href="/documentation?cat=publishersdk"
            ...
            >
              ...
              <span>Attribution Integration Manual</span>
            </Link>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 flex justify-center">
          <div className="relative w-64 h-[450px] rounded-[36px] border-[6px] border-zinc-800 bg-zinc-950 p-3 shadow-2xl shadow-indigo-500/10 flex flex-col justify-between overflow-hidden">
            {/* Phone Notch/Speaker */}
            ...
            <div className="flex-grow flex flex-col justify-between pt-8 pb-4 font-mono text-[9px] text-zinc-400">
              <div className="flex items-center justify-between border-b border-border/40 pb-2">
                <span className="text-[8px] text-primary font-bold">GreatGoga v2.4.0</span>
              ...
              <div className="space-y-4 py-4 flex-grow flex flex-col justify-center">
                <div className="p-3 rounded-xl bg-indigo-950/40 border border-indigo-500/20 text-center space-y-1">
                  <div className="text-[8px] text-indigo-300">TOTAL REWARDS</div>
                  <div className="text-base font-extrabold text-foreground tracking-tight">550.00 PTS</div>
                </div>

                <div className="p-2.5 rounded-xl border border-border bg-card/45 space-y-1">
                  <div className="font-bold text-foreground">Featured Micro-Tasks:</div>
                  <div className="text-[8px] flex items-center gap-1 text-zinc-400">
                    <span className="text-emerald-400">✓</span> Watch promotional feed (+50)
                  </div>
                  <div className="text-[8px] flex items-center gap-1 text-zinc-400">
                    <span className="text-emerald-400">✓</span> Survey attribution (+200)
                  </div>
                </div>
              </div>

              <div className="border-t border-border/40 pt-2 flex items-center justify-between text-[8px] text-muted-foreground font-sans">
                <span>Task attribution: active</span>
                <span>Referral: valid</span>
              </div>
            </div>
      ...
      <section className="space-y-6 mb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {[
            { title: "Secure Local Cache", desc: "Encrypts user points progress locally using Room DB before syncing.", icon: Database },
            { title: "gRPC Telemetry Pipeline", desc: "Transports referrals attribution payloads with high compression protocol.", icon: GitBranch },
            { title: "WorkManager Sync", desc: "Guarantees offline activities sync back to server in background threads.", icon: CheckCircle2 }
          ].map((feat, idx) => {
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start mb-16">
        <div className="md:col-span-7 space-y-6">
          ...
          <div className="p-6 rounded-2xl border border-border bg-muted/10 space-y-3 text-xs">
            ...
            <div className="grid grid-cols-2 gap-4 text-muted-foreground font-mono">
              <div>• Min Android SDK: API 23+</div>
              <div>• Target Android SDK: API 34+</div>
              <div>• Core Protocol: gRPC transport</div>
              <div>• Security: Encrypted Room SQL</div>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-5 space-y-4">
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Database Architecture Flow</h3>
          <div className="p-6 rounded-2xl border border-border bg-zinc-950 flex flex-col items-center gap-4 text-center font-mono text-[10px]">
            ...
            <div className="text-muted-foreground">↓ (WorkManager triggers)</div>
            <div className="w-full p-2.5 rounded bg-cyan-950/40 border border-cyan-500/20 text-cyan-300">
              gRPC Sync Channel
            </div>
            <div className="text-muted-foreground">↓ (flush telemetry)</div>
      ...
      <section className="pt-8 border-t border-border/40 space-y-6">
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-2xl border border-border bg-card/10 space-y-4">
            ...
            {/* Deprecated */}
            {product.changelog.deprecated && (
              <div className="space-y-1.5">
                <span className="text-[10px] font-bold text-red-400 uppercase tracking-widest block">Deprecated</span>
                {product.changelog.deprecated.map((log, lIdx) => (
                  <div key={lIdx} className="text-xs text-muted-foreground font-light leading-relaxed pl-4 relative">
                    <span className="absolute left-0 text-red-400">•</span> {log}
                  </div>
                ))}
              </div>
            )}
